...
<div class="fieldcontain ${hasErrors(bean: organizationInstance, field: 'fekIssue', 'error')} ">
	<label for="fekIssue">
		<g:message code="organization.fekIssue.label" default="Fek Issue" />
		
	</label>
	<g:select id="fekIssue" name="fekIssue.id" from="${com.meerkat.DictionaryItem.list()}" optionKey="id" value="${organizationInstance?.fekIssue?.id}" class="many-to-one" noSelection="['null': '']"/>
